...
ad esempio, che il quadrato costruito sulla diagonale di un quadrato di lato
unitario ha area pari a due unità quadrate. Quindi se il lato del quadrato
ha misura $1$ la misura della diagonale dovrebbe essere rappresentata
da un numero $x$ tale che $x^2=2$. Possiamo facilmente verificare
che non esiste un numero razionale con tale proprietà.

\begin{theorem}[Pitagora]
  Non esiste $x\in \QQ$ tale che $x\cdot x=2$.
\end{theorem}
%
\begin{proof}
  ...
  \[
    2 = x\cdot x= \frac{p\cdot p}{q\cdot q}
  \]
  da cui
  \[
   p\cdot p = 2 q\cdot q.
  \]
  Significa che $p\cdot p$ è pari cioè $2\mid p\cdot p$.
  Essendo $2$ primo significa che $2 \mid p$ e quindi si può
  scrivere $p=2p'$ con $p'\in \ZZ$. Allora abbiamo
  \[
    (2\cdot p)\cdot (2\cdot p) = 2 q \cdot q
  \]
  che è equivalente a
  \[
    2 p\cdot p = q\cdot q.
