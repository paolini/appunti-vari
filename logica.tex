...
\begin{definition}
  ...
  se ogni funzione iniettiva $\sigma\colon A \to A$ è anche suriettiva.
  Diremo che un insieme $A$ è \myemph{infinito}
  se non è finito ovvero se esiste una funzione $\sigma\colon A \to A$
...
su $\NN$.
Il seguente teorema ci dice che gli insiemi finiti sono quelli
che hanno $n$ elementi con $n\in \NN$, e che gli insiemi infiniti
sono quelli che hanno almeno tanti elementi quanti ne ha $\NN$.

\begin{theorem}
Un insieme $A$ è finito se e solo se esiste $n\in \NN$ tale che $\# A = n$.
Un insieme $A$ è infinito se e solo se $\#A \ge \#\NN$.
\end{theorem}
%
\begin{proof}
  Innanzitutto è chiaro che se $\# A = \# B$ allora $A$ è finito/infinito
  se e solo se
  $B$ è finito/infinito.
  Infatti se c'è una corrispondenza biunivoca $\phi\colon A\to B$
  allora ogni funzione $f\colon B \to B$ corrisponde ad una funzione
  $g\colon A \to A$ ponendo $g=f\circ \phi$ e l'iniettività/suriettività di $f$
  è equivalente alla iniettività/suriettività di $g$.

  Dimostriamo allora che $I_n$ è finito per ogni $n\in \NN$. Se esistesse una
  funzione iniettiva ma non suriettiva $f\colon I_n \to I_n$.
  Se fosse $n\not \in f(I_n)$ si avrebbe $f\colon I_n \to I_{n-1}$ iniettiva,
  da cui $n\le n-1$ assurdo. Ma se anche fosse $n=f(k)$ per qualche $k\in I_n$
  potrei definire $g\colon I_n\to I_{n-1}$ come segue. Scelto $x\in I_n\setminus f(I_n)$
  ($x$ esiste in quanto $f$ non è suriettiva) poniamo
  \[
    g(j) =
    \begin{cases}
      f(j) & \text{se $j\neq k$},\\
    ...
    \end{cases}
  \]
  Chiaramente $g$ sarebbe iniettiva e di nuovo otterremmo un assurdo.

  Dunque $I_n$ è finito. Ma se $\# A = \# I_n$ allora anche $A$ è finito.

  Dimostriamo ora che $\#\NN > \# I_n$ per ogni $n\in \NN$.
  Chiaramente $\#\NN\ge \#I_n$ in quanto $\NN\supset I_n$.
  Se per assurdo esistesse $f\colon \NN \to I_n$ iniettiva
  potrei restringere $f$ a $I_{n+1}$
  e avrei una funzione iniettiva $f_{|I_{n+1}}\colon I_{n+1}\to I_n$ da cui
  otterrei l'assurdo $n+1 \le n$.

  Mostriamo ora che se $A$ è infinito allora $\#A \ge \# \NN$.
  Supponiamo dunque che esista $\sigma\colon A \to A$ iniettiva ma non suriettiva.
  Scegliamo $\alpha \in A \setminus \sigma(A)$ e definiamo per induzione $f\colon N \to A$
  con le condizioni:
  \[
    \begin{cases}
        f(0) = \alpha \\
  ...
  Sia $X=\{n\in \NN \colon \exists m\in \NN\colon m\neq n, f(m)=f(n)\}$
  l'insieme dei punti in cui $f$ non è iniettiva.
  Se $f$ non fosse iniettiva tale $X$ sarebbe non vuoto
  e avrebbe quindi un minimo che chiamiamo $m \in X$.
  Per tale $m$ esisterebbe un $n\neq m$ tale che $f(m)=f(n)$. Chiaramente $n\in X$
  e quindi $n>m$ in quanto $m$ è il minimo.
  Non può essere $m=0$ in quanto $f(0) = \alpha$ mentre $f(n)=\sigma(f(n-1)) \neq \alpha$
  visto che $\alpha$ è un valore mai assunto da $\sigma$.
  Ma se $m>0$ allora osserviamo che $\sigma(f(m-1)) = f(m) = f(n) = \sigma(f(n-1))$
  e si avrebbe dunque $m-1\in X$. Questo è assurdo perché $m$ era il minimo di $X$.

  Dimostriamo ora che se $A\subset \NN$ è un insieme tale che non esiste
  $n\in \NN$ per cui $A\subset I_n$ (diremo che $A$ è superiormente illimitato)
  allora $\#A = \#\NN$. Per fare questo definiamo ricorsivamente
  una funzione iniettiva
  $f\colon \NN \to A$ come segue:
  \[
    \begin{cases}
       f(0) = \min A, \\
       f(n+1) = \min (A\setminus I_{f(n)+1}).
    \end{cases}
  \]
  La funzione $f$ è ben definita perché gli insiemi $A$ e $A\setminus I_{f(n)}$
  sono non vuoti e dunque hanno minimo (per il buon ordinamento di $\NN$).
  Chiaramente $f$ è iniettiva in quanto per definizione $f(n+1) > f(n)$.
  Dunque abbiamo mostrato che $\#A \ge \NN$. Ma essendo $A\subset \NN$
  possiamo quindi concludere che $\#A = \#\NN$.

  Più in generale se $\# A \le \#\NN$ esiste un funzione iniettiva
  $f\colon A \to \NN$ e quindi si ha $\# A = \# f(A)$ con $f(A)\subset \NN$.
  Possiamo quindi concludere che se $\#A < \#\NN$ necessariamente deve esistere
  $n\in \NN$ tale che $\#A = n$.
